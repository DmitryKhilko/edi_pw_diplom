...
from data.data_api_persons import *
...
class TestAPIGetPersons:
    ...
    def test_api_can_read_persons(self, user, parameter_description, expected_result):
        ...
        PersonsService.can_read_persons(csrftoken, sessionid, parameter_description, expected_result)
        logging.debug(f'Окончить тест "Получение списка физ.лиц (у роли есть права)" под ролью "{user[0]}"')

    @mark.parametrize('user, parameter_description, expected_result', test_data_api_can_not_read_persons)
    @allure.description(
    ...
    def test_api_can_not_read_persons(self, user, parameter_description, expected_result):
        logging.debug(f'Начать тест "Отказ в получении списка физ.лиц (у роли нет прав)" под ролью "{user[0]}"')
        csrftoken, sessionid = LoginService.login(user, user[0])
        PersonsService.can_not_read_persons(csrftoken, sessionid, parameter_description, expected_result, user[0])
