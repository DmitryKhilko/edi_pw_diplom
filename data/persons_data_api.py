...
from helper.faker import Fake
from settings import *

f = Fake()

# Данные предназначены для параметризованных api-тестов по получению списка физических лиц под ролями,
# которым разрешено получение списка. Структура кортежа:
# users (учетные данные пользователя), message (ответ сервера)
test_data_can_get_persons = (
...
# Данные предназначены для параметризованных api-тестов по получению списка физических лиц под ролями,
# которым запрещено получение списка. Структура кортежа: users (учетные данные пользователя), message (ответ сервера)
test_data_can_not_get_persons = (
...
test_data_can_add_person_valid_param_inside_limits = (
    # Проверка значений параметров внутри границ под АИБ
    (('Администратор ИБ', LOGIN_AIB, PASSWORD_AIB, EMAIL_ACCOUNT_AIB),
     (f.first_name(10, True, ''), f.last_name(10, True, ''), f.patronymic(10, True, ''), f.sex(True, ''),
      f.date(True, ''), f.phone(10, True, ''), f.email(20, True, ''), f.key_id(10, True, ''), f.card_id(10, True, '')),
     (201, 'Created')),
    # Проверка значений параметров внутри границ под АИС
    (('Администратор ИС', LOGIN_AIS, PASSWORD_AIS, EMAIL_ACCOUNT_AIS),
     (f.first_name(10, True, ''), f.last_name(10, True, ''), f.patronymic(10, True, ''), f.sex(True, ''),
      f.date(True, ''), f.phone(10, True, ''), f.email(20, True, ''), f.key_id(10, True, ''), f.card_id(10, True, '')),
     (201, 'Created')),
